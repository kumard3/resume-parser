...
export const divideSectionIntoSubsections = (lines: Lines): Subsections => {
  ...
  if (subsections.length === 1) {
    const isLineNewSubsectionByBold = (line: Line, prevLine: Line) => {
      if (
        !isBold(prevLine[0] as TextItem) &&
        isBold(line[0] as TextItem) &&
        // Ignore bullet points that sometimes being marked as bolded
        // Ignore bullet points that sometimes being marked as bolded
        !BULLET_POINTS.includes(line[0]?.text as string)
      ) {
        return true;
      }
      return false;
...
const createIsLineNewSubsectionByLineGap = (
  ...
  const isLineNewSubsection = (line: Line, prevLine: Line) => {
    const prevLineY = prevLine[0]?.y ?? 0;
    const lineY = line[0]?.y ?? 0;
    return Math.round(prevLineY - lineY) > subsectionLineGapThreshold;
  };
...
const createSubsections = (
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const prevLine = lines[i - 1] || []; // Provide a default value for undefined prevLine
    if (i === 0) {
      subsection.push(line);
      continue;
    }
    if (isLineNewSubsection(line, prevLine)) {
