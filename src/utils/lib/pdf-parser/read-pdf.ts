...
export const handlePdfParse = async (fileUrl: string) => {
  if (fileUrl) {
    const pdfFile = await pdfjs.getDocument(fileUrl).promise;
    let text = "";
    ...
    for (let i = 1; i <= pdfFile.numPages; i++) {
      ...
      const typedContent = content.items as PdfjsTextItemType[];
      const strings = typedContent.map((item) => item.str).join(" ");
      text += strings + "\n";
    }
