...
export const groupTextItemsIntoLines = (textItems: TextItems): Lines => {
  ...
  for (const line of lines) {
    ...
    for (let i = line.length - 1; i > 0; i--) {
      const currentItem = line[i] as TextItem;
      const leftItem = line[i - 1] as TextItem;
...
const shouldAddSpaceBetweenText = (leftText: string, rightText: string) => {
  const leftTextEnd = leftText[leftText.length - 1] as string;
  const rightTextStart = rightText[0] as string;
  const conditions = [
